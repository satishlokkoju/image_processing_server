...
namespace ADTVision 
{
	...
	void geo_verify_f(const cv::Mat &descriptors0, const cv::Mat &points0,const cv::Mat &descriptors1, const cv::Mat &points1, cv::detail::MatchesInfo &matches_info,std::vector<uint32_t> *inliers0, std::vector<uint32_t> *inliers1) 
	{
...
#if 0
		if(descriptors1.rows < 9 || descriptors0.rows < 9 || descriptors0.cols < 128 || descriptors1.cols < 128) 
			return;
#endif
	    ...
	    try 
		{
		    ...
		    for (size_t i = 0; i < matches_info.inliers_mask.size(); ++i) 
			{
				...
				{
		        	if(inliers0)
						inliers0->push_back(matches_info.matches[i].queryIdx);
		        	if(inliers1)
						inliers1->push_back(matches_info.matches[i].trainIdx);
	...
	void geo_verify_h(const cv::Mat &descriptors0, const cv::Mat &points0,const cv::Mat &descriptors1, const cv::Mat &points1, cv::detail::MatchesInfo &matches_info,std::vector<uint32_t> *inliers0, std::vector<uint32_t> *inliers1) 
	{
	    cv::Ptr<cv::flann::IndexParams> indexParams = cv::makePtr<cv::flann::KDTreeIndexParams>();
	    cv::Ptr<cv::flann::SearchParams> searchParams = cv::makePtr<cv::flann::SearchParams>();
	    matches_info.confidence = 0;
	...
	bool compute_sparse_sift_feature(const cv::Mat &img, const std::shared_ptr<const SIFTParams> &params ,cv::Mat &keypoints, cv::Mat &descriptors) 
	{
		...
		cv::Ptr<cv::Feature2D> sift_extractor2 = cv::xfeatures2d::SIFT::create(sift_parameters->max_features, sift_parameters->num_octave_layers, sift_parameters->contrast_threshold, sift_parameters->edge_threshold, sift_parameters->sigma);
		cv::Ptr<cv::Feature2D> sift_extractor = cv::xfeatures2d::SIFT::create();
	...
	bool compute_sparse_surf_feature(const cv::Mat &img ,cv::Mat &keypoints, cv::Mat &descriptors) 
	{
		cv::Ptr<cv::Feature2D> surf_extractor  = cv::xfeatures2d::SURF::create(400,4,2, false, false );

		if(img.size().area() > 0) 
		{	
			std::vector<cv::KeyPoint> keypoint_vec;
			surf_extractor->detect(img, keypoint_vec);
			surf_extractor->compute(img, keypoint_vec, descriptors);
			keypoints = cv::Mat(keypoint_vec.size(), 2, CV_32FC1);
	...
	bool compute_sparse_kaze_feature(const cv::Mat &img ,cv::Mat &keypoints, cv::Mat &descriptors) 
	{
		cv::Ptr<cv::KAZE> kaze = cv::KAZE::create();
		if(img.size().area() > 0) 
		{	
			std::vector<cv::KeyPoint> keypoint_vec;
			kaze->detectAndCompute(img, cv::noArray(), keypoint_vec, descriptors);
			keypoints = cv::Mat(keypoint_vec.size(), 2, CV_32FC1);
	...
	bool compute_sparse_akaze_feature(const cv::Mat &img ,cv::Mat &keypoints, cv::Mat &descriptors) 
	{
		cv::Ptr<cv::AKAZE> akaze = cv::AKAZE::create();
		if(img.size().area() > 0) 
		{	
			std::vector<cv::KeyPoint> keypoint_vec;
			akaze->detectAndCompute(img, cv::noArray(), keypoint_vec, descriptors);
			keypoints = cv::Mat(keypoint_vec.size(), 2, CV_32FC1);
	...
	bool compute_bow_feature(const cv::Mat& descriptors, const cv::Ptr<cv::DescriptorMatcher> &matcher,	cv::Mat& bow_descriptors, std::shared_ptr< std::vector<std::vector<uint32_t> > > cluster_indices) 
	{
	    ...
	    if(cluster_indices != std::shared_ptr< std::vector<std::vector<uint32_t> >  >())
		{
