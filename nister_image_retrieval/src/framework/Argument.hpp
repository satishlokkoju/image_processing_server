...
namespace cloudcv
{
    ...
    protected:
        InputArgument(const std::string& name, const std::string& type) : m_name(name) , m_type(type)
        {
    ...
    protected:
        OutputArgument(const std::string& name, const std::string& type) : m_name(name), m_type(type)
        {
    ...
    protected:
        TypedOutputArgument(const char * name) : OutputArgument(name, typeid(T).name())
        {
    ...
    protected:
        inline RequiredArgument(const char * name) : InputArgument(name, typeid(T).name())
        {
        }
    };
