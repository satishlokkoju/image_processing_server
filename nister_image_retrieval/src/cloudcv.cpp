...
#include "modules/AnalyzeImageAlgorithm.hpp"
#include "modules/IntegralImage.hpp"
#include "modules/ADTAddUser.hpp"
#include "modules/ADTDeleteUser.hpp"
#include "modules/ADTAddImageUser.hpp"
#include "modules/ADTRemoveImageUser.hpp"
#include "modules/ADTRetrieveImageUser.hpp"
...
NAN_METHOD(getInfo)
{
    ...
    if (Nan::Check(info).ArgumentsCount(1).Argument(0).IsString().Bind(algorithmName).Error(&errorMessage))
    {
...
NAN_METHOD(processFunction)
{
    ...
    if (Nan::Check(info).ArgumentsCount(3).
			 ...
			 Argument(1).IsObject().Bind(inputArguments).
                         Argument(2).IsFunction().Bind(resultsCallback).Error(&errorMessage))
    {
        auto algorithm = AlgorithmInfo::Get().find(algorithmName);
        if (algorithm == AlgorithmInfo::Get().end())
        {
...
NAN_MODULE_INIT(RegisterModule)
{
    ...
    AlgorithmInfo::Register(new AnalyzeAlgorithmInfo);
    AlgorithmInfo::Register(new ADTAddUserInfo);
    AlgorithmInfo::Register(new ADTRemoveImageUserInfo);
    AlgorithmInfo::Register(new ADTRetrieveImageUserInfo);
    AlgorithmInfo::Register(new ADTDeleteUserInfo);
    AlgorithmInfo::Register(new ADTAddImageUserInfo);

    Set(target,New<v8::String>("getAlgorithms").ToLocalChecked(),GetFunction(New<v8::FunctionTemplate>(getAlgorithms)).ToLocalChecked());
    Set(target,New<v8::String>("processFunction").ToLocalChecked(),GetFunction(New<v8::FunctionTemplate>(processFunction)).ToLocalChecked());
    Set(target,New<v8::String>("getInfo").ToLocalChecked(),GetFunction(New<v8::FunctionTemplate>(getInfo)).ToLocalChecked());
}
